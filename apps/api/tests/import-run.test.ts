...
async function setup(urls: { importUrl?: string; rosterUrl?: string }, files: Record<string, string | number>) {
  ...
  const emails: Email[] = [];
  let now = T0;
  const deps: Deps = { db, store: new SqlStore(db), apiKey: "k", linkSecret: SECRET, webUrl: "https://digest.example.com", consoleUrl: CONSOLE, adminUrl: "https://admin.example.com", adminEmail: "owner@example.com", slaHours: 48, sendEmail: async (e) => { emails.push(e); }, now: () => now, fetch: fakeFetch(files) };
