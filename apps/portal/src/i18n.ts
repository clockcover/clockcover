// UI copy in both languages for the portal (manager page, payroll accountant's page), the console
...
const M = {
  en: {
    ...
    "api.link invalid or expired": "The link has expired or is not valid.",
    "api.sign in required": "Sign in required.", "api.unauthorized": "Sign in required.",
    "api.email required": "Enter an email address.",
    "api.invalid csv": "The file is not valid CSV.", "api.no rows": "The file has no rows.",
    "api.already resolved": "This gap was already closed.", "api.gap not found": "Gap not found.",
    "api.a note is required": "A note is required — say why the entry will never arrive.",
    "api.an absence needs a note": "An absence needs a note — what happened?",
    "api.outcome must be": "Choose an outcome: approve the hours or report an absence.",
    "api.no import URL configured": "No import URL configured — set one in Settings or upload a file.",
    "api.must be an https:// URL": "Must be an https:// URL.", "api.is not a valid URL": "Not a valid URL.",
    "api.invalid settings": "Some settings are not valid.", "api.invalid employer": "Some employer fields are not valid.",
    "api.employer not found": "Employer not found.", "api.employer has no operator email": "This employer has no operator email.",
    "api.name required": "Name required — what will use this key?", "api.no such active key": "No such active key.",
    "api.from and to must be": "From and to must be dates (YYYY-MM-DD), from before to.",
    "api.internal error": "Server error. Please try again in a moment.",
    // landing (no token)
    ...
    "digest.resolve": "Resolve", "digest.resolveGap": "Resolve gap", "digest.note.label": "Note",
    ...
    "sla.escalated": "Escalated to the payroll accountant — SLA passed", "sla.soon": "Escalates to the payroll accountant in {h} h", "sla.notified": "Notified {when}", "sla.notYet": "Not yet in a digest",
    ...
    "digest.footer.link": "This link is for you — pass it on only to a stand-in · expires {date}",
    "digest.footer.sla": "Unresolved gaps escalate to the payroll accountant after {h} h",
    // escalation page
    ...
    "esc.closed": "✓ Closed — {outcome}", "esc.by.payroll": "by the payroll accountant", "esc.by.manager": "by the manager", "esc.by.record": "a record arrived",
    ...
    "esc.note.label": "Why the entry will never arrive", "esc.close": "Close gap", "esc.already": "This gap was already closed.", "esc.failed": "Could not close the gap.",
    ...
    "c.signin.email": "Operator email", "c.signin.send": "Send sign-in link", "c.signin.expired": "Your sign-in link has expired or is not valid. Request a new one.", "c.signin.api": "Could not reach the API.", "c.signedOut": "Signed out.",
    ...
    "c.ov.escalated": "Escalated · {d} d", "c.ov.closedBy": "{p} closed by the payroll accountant · {r} closed by a later import",
    ...
    "c.imp.exp.lead": "Gaps closed by a manager or by the payroll accountant in the period — approved hours and reported absences with their notes — as CSV, for carrying into your attendance or payroll system.",
    ...
    "c.keys.title": "API keys", "c.keys.lead": "For scripts and schedulers that upload files to the API. A key belongs to this employer only; you see it once — copy it into your scheduler's secret store. Revoke and create a new one to rotate.",
    "c.keys.name.label": "Key name", "c.keys.name": "What will use it? e.g. HR scheduler", "c.keys.create": "Create key", "c.keys.revoke": "Revoke", "c.keys.none": "No keys yet.",
    "c.keys.created": "Copy this key now — it will not be shown again:", "c.keys.lastUsed": "last used {when}", "c.keys.neverUsed": "never used", "c.keys.revoked": "revoked {when}",
    ...
    "a.signin.email": "Owner email", "a.f.operator.short": "Operator email",
    ...
    "tier.team": "Team", "tier.company": "Company", "tier.site": "Site", "tier.larger": "Larger", // product names stay Latin
    "a.tierNote": "Tier is by active employees on the roster (ADR-0006): Team ≤ 50 ($20), Company ≤ 200 ($50), Site ≤ 500 ($100), Larger by agreement.",
  },
  he: {
    "lang.switch": "English",
    ...
    "api.link invalid or expired": "הקישור פג או אינו תקף.",
    "api.sign in required": "נדרשת כניסה.", "api.unauthorized": "נדרשת כניסה.",
    "api.email required": "הזינו כתובת מייל.",
    "api.invalid csv": "הקובץ אינו CSV תקין.", "api.no rows": "בקובץ אין שורות.",
    "api.already resolved": "הפער הזה כבר נסגר.", "api.gap not found": "הפער לא נמצא.",
    "api.a note is required": "חובה להוסיף הערה — למה ההחתמה לא תגיע.",
    "api.an absence needs a note": "היעדרות דורשת הערה — מה קרה?",
    "api.outcome must be": "בחרו תוצאה: אישור שעות או דיווח היעדרות.",
    "api.no import URL configured": "לא הוגדרה כתובת ייבוא — הגדירו אחת בהגדרות או העלו קובץ.",
    "api.must be an https:// URL": "חייבת להיות כתובת https://.", "api.is not a valid URL": "כתובת לא תקינה.",
    "api.invalid settings": "חלק מההגדרות אינן תקינות.", "api.invalid employer": "חלק מפרטי המעסיק אינם תקינים.",
    "api.employer not found": "המעסיק לא נמצא.", "api.employer has no operator email": "למעסיק הזה אין כתובת מפעיל/ה.",
    "api.name required": "נדרש שם — מה ישתמש במפתח?", "api.no such active key": "אין מפתח פעיל כזה.",
    "api.from and to must be": "מתאריך ועד תאריך חייבים להיות תאריכים (YYYY-MM-DD), מתאריך לפני עד תאריך.",
    "api.internal error": "שגיאת שרת. נסו שוב בעוד רגע.",
    ...
    "digest.resolve": "טיפול", "digest.resolveGap": "סגירת הפער", "digest.note.label": "הערה",
    ...
    "sla.escalated": "הועבר לחשב/ת השכר — זמן התגובה עבר", "sla.soon": "יועבר לחשב/ת השכר בעוד {h} שעות", "sla.notified": "הודעה נשלחה {when}", "sla.notYet": "עדיין לא נכלל בתקציר",
    ...
    "digest.footer.link": "הקישור הזה בשבילך — העבירו אותו רק למי שמחליף/ה אותך · תקף עד {date}",
    ...
    "esc.note.label": "למה ההחתמה לא תגיע", "esc.close": "סגירת הפער", "esc.already": "הפער הזה כבר נסגר.", "esc.failed": "לא ניתן לסגור את הפער.",
    ...
    "c.signin.email": "כתובת המפעיל/ה", "c.signin.send": "שליחת קישור כניסה", "c.signin.expired": "קישור הכניסה פג או אינו תקף. בקשו קישור חדש.", "c.signin.api": "אין חיבור ל-API.", "c.signedOut": "יצאת מהמערכת.",
    ...
    "c.ov.escalated": "הועברו לשכר · {d} ימים", "c.ov.closedBy": "{p} נסגרו על ידי חשב/ת השכר · {r} נסגרו בייבוא מאוחר",
    ...
    "c.imp.exp.lead": "פערים שנסגרו על ידי מנהל/ת או חשב/ת השכר בתקופה — שעות שאושרו והיעדרויות שדווחו עם ההערות — כ-CSV, להעברה למערכת הנוכחות או השכר.",
    ...
    "c.keys.title": "מפתחות API", "c.keys.lead": "לסקריפטים ומתזמנים שמעלים קבצים ל-API. מפתח שייך למעסיק הזה בלבד; רואים אותו פעם אחת — העתיקו אותו למאגר הסודות של המתזמן. לרוטציה: בטלו וצרו חדש.",
    "c.keys.name.label": "שם המפתח", "c.keys.name": "מה ישתמש בו? למשל: מתזמן HR", "c.keys.create": "יצירת מפתח", "c.keys.revoke": "ביטול", "c.keys.none": "עדיין אין מפתחות.",
    "c.keys.created": "העתיקו את המפתח עכשיו — הוא לא יוצג שוב:", "c.keys.lastUsed": "שימוש אחרון {when}", "c.keys.neverUsed": "לא היה בשימוש", "c.keys.revoked": "בוטל {when}",
    ...
    "a.signin.email": "כתובת הבעלים", "a.f.operator.short": "כתובת המפעיל/ה",
    ...
    "tier.team": "Team", "tier.company": "Company", "tier.site": "Site", "tier.larger": "Larger",
    "a.tierNote": "המסלול נקבע לפי עובדים פעילים ברשימה (ADR-0006): Team ≤ 50 ($20), Company ≤ 200 ($50), Site ≤ 500 ($100), Larger בהסכמה.",
  },
...
export const t = (key: Key, vars?: Vars): string => fill(M[locale.value][key], vars);
/**
 * What to show for a failed API call: the translation of a known server message (matched by
 * prefix — the API appends details after a colon), else `fallback`. Details from a 400 are kept
 * because they name the offending rows or fields.
 */
export function apiError(e: unknown, fallback: Key = "error.generic"): string {
  if (!(e instanceof ApiError)) return t(fallback);
  const known = API_KEYS.find((k) => e.message.startsWith(k.slice(4)));
  if (!known) return t(fallback);
  const details = e.status === 400 ? e.message.slice(e.message.indexOf(": ") + 2) : "";
  return details && e.message.includes(": ") ? `${t(known)} ${details}` : t(known);
}
const API_KEYS = (Object.keys(M.en) as Key[]).filter((k) => k.startsWith("api."));

/** Pure variant for tests and non-reactive code. */
export const tr = (l: Locale, key: Key, vars?: Vars): string => fill(M[l][key], vars);
