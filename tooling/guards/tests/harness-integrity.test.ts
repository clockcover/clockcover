...
test("appliesTo covers guards, hooks, CI and agent config", () => {
  for (const p of [
    ...
    "package.json", "turbo.json", ".markdownlint-cli2.jsonc", ".gitignore",
    ".huskyrc", "../../.config/husky/init.sh", "/home/x/.config/husky/init.sh", "../.huskyrc",
  ]) assert.ok(appliesTo(p), p);
  for (const p of ["docs/contributing.md", "packages/core/src/x.ts", "apps/api/package.json", "apps/api/tsconfig.json", "apps/web/CLAUDE.md", "apps/api/.gitignore"]) {
    assert.ok(!appliesTo(p), p);
  }
});
...
test("checkCommand flags hook bypasses", () => {
  for (const c of [
    ...
    "git -c core.hooksPath /dev/null push", "chmod -R a-x .husky", "chmod 000 .claude", "echo 'HUSKY=0' > ~/.config/husky/init.sh",
    "echo 'HUSKY=0' >> ~/.huskyrc", "mkdir -p $HOME/.config/husky", "ls\ngit commit --no-verify -m x",
