...
and escalate to the payroll accountant only if that manager doesn't act
within an SLA.

## Goals

- Daily, produce a **personal digest per department manager**: their
  employees' missing/incomplete clock entries for the period. (A weekly
  cadence is a later option, not built — `open-questions.md`.)
...
- Every resolution says **what happened**: the employee was present (the
  ...
  needs an explanation). Managers choose from the digest; the payroll
  accountant from the escalation; a record arriving with a later import
  means present.
...
- A public **marketing website** (`apps/web`): what the product does,
  ...
  data.
- An **operator console** (`console.clockcover.com`, ADR-0005) for the
  person who runs ClockCover at an employer: sign in by emailed link,
  upload the roster and the exports, edit employer settings (timezone,
  SLA, emails, language), issue upload API keys (ADR-0007), read the
  "manager acted within SLA" metric.
...
- **Export of corrections**: a CSV of gaps closed by a manager or by
  the payroll accountant in a period — approved hours and reported
  absences with notes — for carrying into the attendance or payroll
  system.
...
- **Two languages, Hebrew and English** (ADR-0008). Every email and
  every page a person sees follows the employer's language
  (`employers.locale`, set by the owner at creation and by the operator
  in Settings); Hebrew is
